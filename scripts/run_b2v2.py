#!/usr/bin/env python3
"""
B2-v2: Cell-Level CVaR Repair — 4 methods, matched compute, 3 seeds.

Methods:
  cvar_cell   — Cell-Level Dual CVaR DRO (Rockafellar-Uryasev on Mondrian cells)
  jtt_cell    — JTT-Cell (two-stage: ERM warm-start → cell-targeted upweight)
  cell_only   — Uniform upweight of worst-k cells (ablation: no CVaR objective)
  global      — Global FT matched budget (control)

Design decisions (from literature review + GPT-5.4 critical review):
  - Matched compute: all methods get identical total gradient updates
  - Cell rescore: every RESCORE_INTERVAL fraction of an epoch (not per-step EMA)
  - η: exact empirical quantile of cell losses (not SGD-learned)
  - Gradient multiplier clip to prevent single-cell domination
  - 3-seed minimum for any publishable claim

Usage:
  # Single run
  CUDA_VISIBLE_DEVICES=0 python scripts/run_b2v2.py --method cvar_cell --seed 1

  # Full sweep (recommended)
  for method in cvar_cell jtt_cell cell_only global; do
    for seed in 1 2 3; do
      CUDA_VISIBLE_DEVICES=0 python scripts/run_b2v2.py --method $method --seed $seed &
    done
  done
  wait

References:
  - Rockafellar & Uryasev (2000): CVaR dual formulation
  - Sagawa et al. (ICLR 2020): Group DRO exponentiated gradient
  - Liu et al. (ICML 2021): JTT two-stage
  - Li et al. (ICLR 2021): Tilted ERM
  - LaBonte et al. (NeurIPS 2024): JTT can beat dynamic CVaR DRO
"""
import argparse
import json
import os
...
OUT_DIR    = Path("results/sprint2/b2v2")
...
# ═══════════════════════════════════════════════════════════════════════
def main():
    parser = argparse.ArgumentParser(description="B2-v2: Cell-Level CVaR Repair")
    ...
    run_name = f"b2v2_{args.method}_seed{args.seed}"
    ...
    if use_wandb:
        wandb.init(
            ...
            config=vars(args),
            tags=["b2v2", args.method, f"seed{args.seed}"],
        )

    print("=" * 60)
    print(f"B2-v2: {args.method} (seed={args.seed})")
    ...
    if args.method == "cvar_cell":
        cvar_weighter = CellCVaRWeighter(
            partition, alpha=args.cvar_alpha,
    ...
    if args.method in ("cvar_cell", "cell_only", "jtt_cell"):
        ...
        print(f"  Estimated {len(init_cell_losses)} cells")
    ...
    warmup_rescore_done = (args.method != "cvar_cell")  # only cvar_cell needs this
        ...
        for micro_step in range(n_train):
            ...
            if (args.method == "cvar_cell"
                ...
                print(f"\n  [CVaR] Warm-start ended at step {global_step}. "
                      f"Rescoring cells with warm-started model...")
    ...
    with open(run_dir / "b2v2_results.json", "w") as f:
        json.dump(summary, f, indent=2)

    # ── Print summary ──
    print(f"\n{'='*60}")
    print(f"B2-v2 COMPLETE: {args.method}")
    print(f"{'='*60}")
